...
Application::Application() {
	...
	box.GetComponent<BoxCollider2D>()->setSize(0.2, 1);
	box.AddComponent<Box>();
	box.GetComponent<Box>()->setSize(0.2, 1);
	box.GetComponent<Box>()->fill();
	box.GetComponent<Box>()->setColor(255, 0, 0, 255);
	box.AddComponent<Rigidbody2D>();
	box.GetComponent<Transform2D>()->setPosition(Vector2D(Graphics::Instance()->SCREEN_WIDTH / 2.0 + 50, Graphics::Instance()->SCREEN_HEIGHT / 2 - 150.0), true);
	...
	box2.GetComponent<Rigidbody2D>()->setVelocity({ -8, -3 });
	box2.GetComponent<Rigidbody2D>()->setAngularSpeed(0);
	box2.GetComponent<Transform2D>()->setRotation(-30);
	...
	wall.GetComponent<Box>()->setSize(0.5, 3);
